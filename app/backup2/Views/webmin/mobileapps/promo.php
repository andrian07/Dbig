                        ...
                        <table id="tblmobilepromo" class="table table-bordered table-hover" width="100%">
                            <thead>
                                <tr>
                                    ...
                                    <th data-priority="4">Tanggal Mulai</th>
                                    <th data-priority="5">Tanggal Akhir</th>
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <input id="old_promo_image" name="old_promo_image" value="" type="hidden">
...
<script>
    $(document).ready(function() {
        ...
        let tblmobilepromo = $("#tblmobilepromo").DataTable({
            ...
            ajax: {
                url: base_url + '/webmin/mobileapps/promo/table',
                type: "POST",
                error: function() {
        ...
        function addMode() {
            ...
            formMode = 'add';
            $('#old_promo_image').val('');
            $('#mobile_promo_id').val('0');
            $('#mobile_promo_title').val('');
            $('#mobile_promo_desc').val('');
            $('#mobile_promo_start_date').val('');
            $('#mobile_promo_end_date').val('');
            $('#active').val('Y');
            clearUploadImage();
        ...
        function editMode(data) {
            ...
            formMode = 'edit';
            $('#old_promo_image').val(htmlEntities.decode(data.mobile_promo_image));
            $('#mobile_promo_id').val(data.mobile_promo_id);
            $('#mobile_promo_title').val(htmlEntities.decode(data.mobile_promo_title));
            $('#mobile_promo_desc').val(htmlEntities.decode(data.mobile_promo_desc));
            $('#mobile_promo_start_date').val(data.mobile_promo_start_date);
            $('#mobile_promo_end_date').val(data.mobile_promo_end_date);
            $('#active').val(data.active);
            clearUploadImage();
            $('#mobile_promo_image').attr('src', htmlEntities.decode(data.image_url));
            $('#modal-addpromo').modal(configModal);
        }
        ...
        $('#btnsave').click(function(e) {
            ...
            if (form.parsley().isValid()) {
                ...
                let actUrl = base_url + '/webmin/mobileapps/promo/save/add';

                if (formMode == 'edit') {
                    question = 'Yakin ingin memperbarui data Promo?';
                    actUrl = base_url + '/webmin/mobileapps/promo/save/edit';
                }
                ...
                message.question(question).then(function(answer) {
                    ...
                    if (yes) {
                        ...
                        let file = $('#upload_image');
                        formValues.append('mobile_promo_id', $('#mobile_promo_id').val());
                        ...
                        formValues.append('old_promo_image', $('#old_promo_image').val());
        ...
        $("#tblmobilepromo").on('click', '.btnedit', function(e) {
            ...
            let actUrl = base_url + '/webmin/mobileapps/promo/getbyid/' + id;
        ...
        $("#tblmobilepromo").on('click', '.btndelete', function(e) {
            ...
            let actUrl = base_url + '/webmin/mobileapps/promo/delete/' + id;
            message.question(question).then(function(answer) {
                let yes = parseMessageResult(answer);
                if (yes) {
                    ajax_get(actUrl, null, {
