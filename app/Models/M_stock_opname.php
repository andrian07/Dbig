...
class M_stock_opname extends Model
{
    ...
    public function opnameProductV2($warehouse_id, $product_id = [], $user_id)
    {
        // V2: GROUP BY DATE //
        $save = ['success' => FALSE, 'message' => 'Gagal menambahkan data produk'];
        ...
        foreach ($getStock as $ws) {
            ...
            $exp_date       = $ws['exp_date'];
            $stock          = floatval($ws['stock']);
            $base_cogs      = $ws['base_cogs'];
            $product_key    = md5($exp_date . $user_id . $pid . $stock_id . $warehouse_id);
            if (isset($tempData[$product_key])) {
                $tempData[$product_key]['temp_warehouse_stock']    += $stock;
                $tempData[$product_key]['temp_system_stock']       += $stock;
            } else {
                $tempData[$product_key] = [
                    'product_key'           => $product_key,
                    'product_id'            => $pid,
                    'stock_id'              => $stock_id,
                    'warehouse_id'          => $warehouse_id,
        ...
        if (count($find_product) > 0) {
            ...
            foreach ($getProduct as $prd) {
                $pid        = $prd['product_id'];
                $base_cogs  = $prd['base_cogs'];
                $product_key    = md5(date('Ymd') . $user_id . $pid . 0 . $warehouse_id);
                $tempData[$product_key] = [
                    'product_key'           => $product_key,
                    'product_id'            => $pid,
                    'stock_id'              => 0,
    ...
    public function insertOpnameV2($data, $return_opname_id = false)
    {
        $opname_id = 0;
        $this->db->query('LOCK TABLES hd_opname WRITE,dt_opname WRITE,temp_opname WRITE,ms_product_stock WRITE,ms_warehouse_stock WRITE,last_record_number WRITE');
        $sqlUpdateLastNumber = NULL;
            ...
            foreach ($getTemp as $row) {
                ...
                if ($stock_add != 0) {
                    $getWarehouseStock = $this->db->table('ms_warehouse_Stock')
                        ...
                        ->where("stock>", 0)
                        ->orderBy("stock_id", "DESC")->get()->getResultArray();

                    if ($stock_add > 0) {
                        //kelebihan stok
                        if (count($getWarehouseStock) > 0) {
                            //ada stock, update row baris 1 (0)
                            $updateStockId = $getWarehouseStock[0]['stock_id'];
                            $updateStock = floatval($getWarehouseStock[0]['stock']) + $stock_add;
                            $data_update_warehouse_stock[] = [
                                'stock_id'      => $updateStockId,
                                'stock'         => $updateStock
                            ];
                        } else {
                            //gak ada stock, insert
                            $data_insert_warehouse_stock[] = [
                                'product_id'    => $product_id,
                                'warehouse_id'  => $warehouse_id,
                                'purchase_id'   => 0,
                        ...
                        //kekurangan stok
                        $stock_reduce = $stock_add * -1;
                        foreach ($getWarehouseStock as $warehouseStock) {
                            if ($stock_reduce > 0) {
                                $curStock = floatval($warehouseStock['stock']);
                                $updateStockId = $warehouseStock['stock_id'];
                                if ($curStock >= $stock_reduce) {
                                    //stok >= jumlah pot stock_reduce, potong senilai stock_reduce
                                    $updateStock    = $curStock - $stock_reduce;
                                    $stock_reduce   = 0;
                                    $data_update_warehouse_stock[] = [
                                        'stock_id'      => $updateStockId,
                                        'stock'         => $updateStock
                                    ...
                                    //stok < jumlah pot stock_reduce, potong semua stock
                                    $updateStock    = 0;
                                    $stock_reduce   = $stock_reduce - $curStock;
                                    $data_update_warehouse_stock[] = [
                                        'stock_id'      => $updateStockId,
                                        'stock'         => $updateStock
                                    ];
