                        ...
                        <form id="frmcategory" class="form-horizontal">
                            <div class="modal-body">
                                ...
                                <div class="form-group">
                                    <label for="category_name" class="col-sm-12">Nama Brand</label>
                                    <div class="col-sm-12">
                                        <input type="text" class="form-control" id="category_name" name="category_name" placeholder="Nama Brand" value="" data-parsley-maxlength="200" data-parsley-trigger-after-failure="focusout" data-parsley-vcategoryname required>
                                    </div>
                                </div>
                                <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        function addMode() {
            ...
            $('#title-frmcategory').html('Tambah Brand');
            form[0].reset();
            form.parsley().reset();
            formMode = 'add';
            $('#category_id').val('0');
        ...
        function editMode(data) {
            let form = $('#frmcategory');
            $('#title-frmcategory').html('Ubah Brand');
            form[0].reset();
            form.parsley().reset();
            formMode = 'edit';
            $('#category_id').val('1');
            $('#category_name').val('TOTO');
            $('#category_description').val('');
            $('#modal-category').modal(configModal);
        }
        ...
        $("#tblcategory").on('click', '.btndelete', function(e) {
            ...
            let question = 'Yakin ingin menghapus brand <b>TOTO</b>?';

            message.question(question).then(function(answer) {
                let yes = parseMessageResult(answer);
                if (yes) {
                    notification.success('Brand telah dihapus');
                }
            })
        })
